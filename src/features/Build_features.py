...
def build_all_features(df, horizon=24):
    df = crear_lags(df)
...
def crear_target_futuro(df, horizon=24):
    ...
    """
    df[f'O3_t{horizon}h'] = df.groupby('station_id')['O3'].shift(-horizon)
    # Eliminamos los registros donde el target es NaN
    # (las últimas N horas de cada estación no tienen target)
    df = df.dropna(subset=[f'O3_t{horizon}h'])
    return df, f'O3_t{horizon}h'